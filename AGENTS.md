...
Before finishing, run `pixi run -e dev ruff check --fix .`, `pixi run -e dev ruff format .`, and `pixi run -e all ty check`. Do not write tests unless asked.Only run focused tests, not the entire test suite. 
